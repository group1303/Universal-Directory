            ...
            function getProds(prodId) {
                $http.get('http://localhost:8080/getProds' + prodId)
                    .success(function (data, status, headers, config) {
                        ...
                        //console.log($scope.prodsItems);
                        noDuplicateParams($scope.prodsItems);
                    })
                    .error(function (error, status, headers, config) {
                        console.log(status);
                        console.log("getProds error");
                    });
            }
            function noDuplicateParams(prodsItems){
                var uniquePars = [];
                prodsItems.forEach(function(item, i , arr){

                    item[0].prodParams.forEach(function(item,i,arr){
                        var self = item;
                        if($jq.inArray('value_' + i,uniquePars) && i < 1){
                            var nameP = item.namePar;
                            var valueP = [];
                            uniquePars['value_' + i] = [];
                            valueP.push(item.valuePar);
                            uniquePars['value_' + i].push({nameP:nameP, valueP: valueP});
                        }else{
                            var k = 0;
                            uniquePars['value_' + k].forEach(function(item,i,arr){
                                //console.log(self);
                                if($jq.inArray(self.valuePar,item.valueP)){
                                    //console.log(item);
                                    item.valueP.push(self.valuePar);
                                    k++;
                                }else {
                                    k++;
                                }
...
// };
            $scope.checkBox = {
                ...
                //text: 'prodsItems[0].prodParams[0].namePar',
                value: false
            }
            $scope.selectedItem = {
                name: '',
