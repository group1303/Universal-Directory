...
var classes = 'SELECT id_class,short_name,name,main_class,base_ei FROM chem_class;',
    ...
    prodImg = 'select id_prod, image from prod where id_prod=';
/**
 * query
    ...
    db.queryDB(str1, function (dataOne) {
        dataOne.forEach(function (item, i, arr) {
            ...
            db.queryDB(str2, function (dataTwo) {
                ...
                arrProdPars.push([{productID, productName,productClassName, prodParams}]);
                prodParams = [];
            });
        });
        setTimeout(function () {
...
router.get('/getClasses:id', function (req, res, next) {
    query("select id_class,name,main_class from chem_class where main_class=" + req.params.id, res);
});

router.get('/prodImg:id', function (req, res, next) {
    getProdImage(prodImg, req.params.id, res);
});
